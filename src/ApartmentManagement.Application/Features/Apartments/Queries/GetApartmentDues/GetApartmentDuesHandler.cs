...
public class GetApartmentDuesHandler : IRequestHandler<GetApartmentDuesQuery, Result<List<DueDto>>>
{
    private readonly IApplicationDbContext _db;

    public GetApartmentDuesHandler(IApplicationDbContext db) => _db = db;

    public async Task<Result<List<DueDto>>> Handle(GetApartmentDuesQuery request, CancellationToken ct)
    {
        ...
        return Result<List<DueDto>>.Success(dues);
    }
}
